...
public abstract class DeleteFilter<T> {
  ...
  private CloseableIterable<T> applyPosDeletes(
    ...
    if (posIndex.isPresent()
        || posDeletes.stream().mapToLong(DeleteFile::recordCount).sum() < setFilterThreshold) {
  ...
  @FunctionalInterface
  public interface DeletePosIndexCacheLoader<T> {
    T load(String key, Callable<T> fn);
  }

  public Map<String, PositionDeleteIndex> createPosIndexMap(
      DeletePosIndexCacheLoader<Map<String, PositionDeleteIndex>> posIndexCache,
      Iterable<FileScanTask> fileTasks) {

...
public abstract class DeleteFilter<T> {
  ...
  public CloseableIterable<T> filter(
      CloseableIterable<T> records, Map<String, PositionDeleteIndex> positionIndexMap) {
    ...
    return applyEqDeletes(!skipPosDeletes ? applyPosDeletes(records, positionIndex) : records);
  }
  ...
  private CloseableIterable<T> applyPosDeletes(
    ...
    if (posDeletes.stream().mapToLong(DeleteFile::recordCount).sum() < setFilterThreshold) {
      PositionDeleteIndex pi =
          positionIndex.orElseGet(() -> Deletes.toPositionIndex(filePath, deletes));
  ...
  public Map<String, PositionDeleteIndex> createPosIndexMap(
    ...
    if (posDeleteFiles.stream().mapToLong(DeleteFile::recordCount).sum() < filterThreshold) {
      ...
      Tasks.range(posDeleteFiles.size())
          .stopOnFailure()
          .throwFailureWhenFinished()
          // .executeWith(ThreadPools.getDeleteWorkerPool())
          .run(
              index -> {
                DeleteFile deleteFile = posDeleteFiles.get(index);
  ...
  private static List<DeleteFile> distinctPosDeletes(Iterable<FileScanTask> fileTasks) {
    Set<CharSequence> deleteSet = CharSequenceSet.empty(Comparators.filePath());

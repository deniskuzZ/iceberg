...
public class IcebergInputFormat<T> extends InputFormat<Void, T> {
  ...
  private static final class IcebergRecordReader<T> extends RecordReader<Void, T> {
    ...
    private static final Cache<String, Map<String, PositionDeleteIndex>>
    ...
    private CloseableIterator<T> nextTask() {
      Function<DeleteFilter<T>, Map<String, PositionDeleteIndex>> posIndex =
          filter -> filter.createPosIndexMap((String path, Callable<Map<String, PositionDeleteIndex>> callable) ->
                  DELETE_POS_INDEX_CACHE.get(path, fn -> {
                    try {
      ...
      switch (inMemoryDataModel) {
        ...
        case GENERIC:
          ...
          return deletes.filter(
              openTask(currentTask, requiredSchema), posIndex.apply(deletes));
        default:
